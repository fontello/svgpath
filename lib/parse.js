...
function scanParam(state) {
  ...
  if (index >= max) {
    state.err = 'can\'t consume data at pos ' + index;
    return;
  }
  ...
  if (ch !== 0x2E/* . */) {
    zeroFirst = (ch === 0x30/* 0 */ && index === start);
    index++;
...
function scanMandatoryParams(state) {
  ...
  while (state.index < max && state.path.charCodeAt(state.index) === 0x2C/* , */) {
    ...
    skipSpaces(state);
    if (state.index > max) {
      state.err = '`,` must be followed by number';
      return;
    }
    scanParam(state);
...
function scanSegment(state) {
  ...
  if (state.index >= max) {
    state.err = 'can\'t consume data at pos ' + state.index;
    return;
  }
