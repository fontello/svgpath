...
SvgPath.prototype.__matrix = function (matrix) {
  var self = this;
  ...
  this.iterate(function (segment, index, x, y) {
    ...
    switch (segment[0]) {
      ...
      case 'v':
        point  = matrix.calc(0, segment[1], true);
        result = (point[0] === 0) ? [ 'v', point[1] ] : [ 'l', point[0], point[1] ];
        break;

      case 'V':
        prev   = matrix.calc(x, y, false);
        point  = matrix.calc(x, segment[1], false);
        result = (point[0] === prev[0]) ? [ 'V', point[1] ] : [ 'L', point[0], point[1] ];
        break;

      case 'h':
        point  = matrix.calc(segment[1], 0, true);
        result = (point[1] === 0) ? [ 'h', point[0] ] : [ 'l', point[0], point[1] ];
        break;

      case 'H':
        prev   = matrix.calc(x, y, false);
        point = matrix.calc(segment[1], y, false);
      ...
      case 'a':
        ...
        point  = matrix.calc(segment[1], segment[2], true);
        result[1] = point[0];
        result[2] = point[1];
        point  = matrix.calc(segment[6], segment[7], true);
        result[6] = point[0];
        result[7] = point[1];
        break;
      ...
      case 'A':
        ...
        result[1] = point[0];
        result[2] = point[1];
        point  = matrix.calc(segment[6], segment[7], false);
        result[6] = point[0];
        result[7] = point[1];
        break;
      ...
      default:
        ...
        for (var i = 1; i < segment.length; i += 2) {
          point = matrix.calc(segment[i], segment[i + 1], isRelative);
